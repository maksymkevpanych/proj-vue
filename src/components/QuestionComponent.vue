<template>
  <div class="quest">
    ...
    <p v-else>
      Тестування завершено! Ваш результат: {{ score }} / {{ tests.length }}
      <br />
...
export default {
  computed: {
    ...
    isFinished() {
      return this.finished || this.user.currentTestIndex >= this.tests.length;
    },
  },
  data() {
    return {
  ...
  methods: {
    ...
    showResultAlert(answer) {
      if (answer === this.currentTest.correctAnswer) {
        ...
        alert(
          `Відповідь неправильна! Правильна відповідь: ${this.currentTest.correctAnswer}`
        );
      }
    },
    reset() {
